...
find.sister <- function(tr, clade, allow.polytomy = T){
    ...
    if(allow.polytomy & is.polytomy(tr, mrca)){
        tips <- 1:length(tr$tip.label)
        all_descendants <- get.descending.nodes.branches(tr, mrca)$descending.nodes
        tips_in_poly_clade <- tips[tips %in% all_descendants]
        clade_index <- match(clade, tr$tip.label)
        print('Note that in polytomies, the sister are all the taxa not included in the clade')
        return(tips_in_poly_clade[ -clade_index])
    }else{
        tips <- 1:length(tr$tip.label)
...
isMonophyletic <- function(tr, taxa){
    if(is.monophyletic(tr, taxa)){
        return(TRUE)
    }else{
        mrca <- get.mrca(tr, taxa)
        if(is.polytomy(tr, mrca)){
            descendants <- tr$edge[tr$edge[, 1] == mrca, 2]
            descendingTips <- descendants[descendants %in% 1:length(tr$tip.label)]
            if(all(taxa %in% tr$tip.label[descendingTips])){ #If all are terminal
                return(TRUE)
            }else{# If not all are terminal
                taxaPrune <- taxa[!taxa %in% tr$tip.label[descendingTips]]
                descendingNodes <- descendants[!descendants %in% 1:length(tr$tip.label)]
                for(i in 1:length(descendingNodes)){#iterate nodes and check if all ancestors are in
                    allNodesTemp <- get.descending.nodes.branches(tr, descendingNodes[i])$descending.nodes[-1]
                    allTipsTemp <- tr$tip.label[allNodesTemp][allNodesTemp %in% 1:length(tr$tip.label)]
                    if(all(allTipsTemp %in% taxaPrune)){
                        taxaPrune <- taxaPrune[!taxaPrune %in% allTipsTemp]
                    }
                }
                if(length(taxaPrune) == 0){
                    return(TRUE)
                }else{
                    return(FALSE)
                }
